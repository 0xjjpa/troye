...
import { EarnReponse } from './api/earn'
const { getHash } = require('emoji-hash-gen')

const Earn = () => {
  const [isLoading, setLoading] = useState(false)
  const [response, setResponse] = useState<EarnReponse | null>(null)
  ...
  const [publicKeyAsHexAndHashed, setPublicKeyAsHexAndHashed] = useState<
    string | null
  >(null)
  const [troyePublicKey, setTroyePublicKey] = useState<ArrayBuffer | null>(null)
  const [troyePublicKeyAsHexHashed, setTroyePublicKeyAsHexHashed] = useState<
    string | null
  >(null)
  ...
  useEffect(() => {
    if (qrcodePayload) {
      setPublicKeyAsHexAndHashed(
        getHash(qrcodePayload.publicKeyAsHex || 'empty')
      )
    }
    return () => {
  ...
  async function sendVerification() {
    try {
      ...
      const response = await fetch('/api/earn', {
        ...
        body: JSON.stringify({
          ...verification,
          clientDataJSON: buf2hex(verification.clientDataJSON),
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Troye is a platform to create loyalty programs"
        />
      ...
      <main style={{ position: 'relative' }}>
        ...
        <Flex
          ...
          alignItems="center"
        >
          <Button onClick={() => setIsCameraDisplayed(!isCameraDisplayed)}>
            {!isCameraDisplayed ? 'Display' : 'Hide'} Camera
          </Button>
          {isCameraDisplayed && (
            <BarcodeScanner setBarcodeValue={setQRCodeValue} />
          )}
          {qrcodePayload && (
            <Flex
              direction="column"
            ...
            >
              ...
              {troyePublicKeyAsHexHashed && (
                <Flex direction="column">
                  ...
                  <Text color="gray.400" fontSize="sm">
                    Your Troye Id Public Identifier
                  </Text>
                </Flex>
              )}
              <TroyeId
                ...
                qrPayloadAsUint8Array={new TextEncoder().encode(
          ...
          {verification?.isValid && (
            <>
              <Button
                ...
                colorScheme="green"
              >
                Send Signed Troye Proof
              </Button>

              {response &&
                (response?.status == 'ok' ? (
                  <Text>
                    Signed submission sent, see{' '}
                    <ChakraLink
                    style={{ textDecoration: 'underline' }}
                      isExternal={true}
                      href={`https://goerli-optimism.etherscan.io/tx/${response?.transactionHash}`}
                    >
                      transaction.
                    </ChakraLink>
                  </Text>
                ) : (
                  <Text>`Error sending submission: ${response?.message}` </Text>
                ))}
            </>
          )}
        </Flex>
